...
**既定は `なし`（補正しない）** で、勝手に色が変わることはありません。淡い色が拾えないときだけ上げてください。

設定項目は折りたたんであり、見出しの「🎨 色の調整」を押すと下に開きます。
一度設定すればブラウザに保存され、次の取り込みにもそのまま使われるので、**毎回開く必要はありません**。
閉じているときは見出しの右に **現在の設定と「残る画素」の割合**（例: `ふつう・ピンク2.2倍｜残る画素 12.3%`）が
出るので、開かずに「今の設定で絵が拾えているか」を確認できます。
